...
def logPriorIncu(mainControl,meanPar,stdPar):
    logPrior = stats.norm.logpdf(meanPar,mainControl.priorIncu["mean"].mean(),scale=mainControl.priorIncu["mean"].std())+stats.norm.logpdf(stdPar,mainControl.priorIncu["sd"].mean(),scale=mainControl.priorIncu["sd"].std())
    return logPrior


def logPriorDelay(mainControl,meanPar,stdPar):
    logPrior = stats.norm.logpdf(meanPar,mainControl.priorDelay["mean"].mean(),scale=mainControl.priorDelay["mean"].std())+stats.norm.logpdf(stdPar,mainControl.priorDelay["sd"].mean(),scale=mainControl.priorDelay["sd"].std())
    return logPrior
...
# Until time
timeArr = list(range(int(max(outbreakData.confirmation_avg_date)),11,-1))
