...
if not os.path.isfile("figure3gh_rec_latin_hypercube_samples.csv"):
    ...
    rec.to_csv("figure3gh_rec_latin_hypercube_samples.csv", header=None, index=False)
else:
    rec = pd.read_csv("figure3gh_rec_latin_hypercube_samples.csv",header=None)


# Generation time
meanGen = [5,7]
stdGen = [5,7]
if not os.path.isfile("figure3gh_gen_latin_hypercube_samples.csv"):
    gen = [meanGen[0]+(meanGen[1]-meanGen[0])*lhSample[:,5], stdGen[0]+(stdGen[1]-stdGen[0])*lhSample[:,6]]
    gen = pd.DataFrame(gen).T
    gen.to_csv("figure3gh_gen_latin_hypercube_samples.csv", header=None, index=False)
else:
    gen = pd.read_csv("figure3gh_gen_latin_hypercube_samples.csv",header=None)
...
for iiScn in range(len(recComplete)):
    if not os.path.isfile("figure3gh_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1)):
...
#     generation time
        ...
        for i in range (math.ceil(simParms[5]*dis)):
        ...
        allRecTable.to_csv("figure3gh_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1), index=False)
