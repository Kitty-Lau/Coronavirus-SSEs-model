...
sampler = stats.qmc.LatinHypercube(d=numParms)
...
if not os.path.isfile("figure3ef_rec_latin_hypercube_samples.csv"):
    rec = [meanIncuRange[0]+(meanIncuRange[1]-meanIncuRange[0])*lhSample[:,0], stdIncuRange[0]+(stdIncuRange[1]-stdIncuRange[0])*lhSample[:,1], percInfRange[0]+(percInfRange[1]-percInfRange[0])*lhSample[:,2]]
    rec = pd.DataFrame(rec).T
    rec.to_csv("figure3ef_rec_latin_hypercube_samples.csv", header=None, index=False)
else:
    rec = pd.read_csv("figure3ef_rec_latin_hypercube_samples.csv",header=None)


# Generation time
meanGen = [5,7]
stdGen = [5,7]
if not os.path.isfile("figure3ef_gen_latin_hypercube_samples.csv"):
    gen = [meanGen[0]+(meanGen[1]-meanGen[0])*lhSample[:,3], stdGen[0]+(stdGen[1]-stdGen[0])*lhSample[:,4]]
    gen = pd.DataFrame(gen).T
    gen.to_csv("figure3ef_gen_latin_hypercube_samples.csv", header=None, index=False)
else:
    gen = pd.read_csv("figure3ef_gen_latin_hypercube_samples.csv",header=None)
...
for iiPrior in range(len(numPriorVec)):
    ...
    for iiSource in range(len(numPointSourceVec)):
        ...
        for iiScn in range(numScn):
            ...
            recComplete.loc[kkText,3] = numPrior
            recComplete.loc[kkText,4] = numPointSource
...
class createConstants_nodelay: 
    def __init__(self, numCluster, numMeanIncubation, distrIncubation, clusterExposure):
...
def loglikelihood_nodelay(parms,incuData,mainControl):
    ...
    for iiCluster in range(len(clusterSize)):
        try:
            logLobs = math.log(max(stats.binom.pmf(math.ceil(clusterSize[iiCluster]), math.ceil(clusterSize[iiCluster]+len(incuData)), 1-stats.lognorm.cdf(mainControl.dateUntil, incuParaB, scale = math.exp(incuParaA))), 1e-20))-len(incuData)*math.log(max(stats.lognorm.cdf(mainControl.dateUntil, incuParaB, scale = math.exp(incuParaA)),1e-20))
        except OverflowError:
            logLobs = math.log(max(float('0'), 1e-20))-len(incuData)*math.log(max(stats.lognorm.cdf(mainControl.dateUntil, incuParaB, scale = math.exp(incuParaA)),1e-20))
        except ValueError: 
            logLobs = math.log(max(float('nan'), 1e-20))-len(incuData)*math.log(max(stats.lognorm.cdf(mainControl.dateUntil, incuParaB, scale = math.exp(incuParaA)),1e-20))
        logLobsIncu = []
        ...
        logL[iiCluster] = logLobs+sum(np.array(logLobsIncu))  
    return sum(logL)


def mcmcParallel_nodelay(mcSteps,parameters,incuData,mainControl,parameterSteps,lowerLimit,upperLimit):
    goodMC = False
    ...
    #     current loglikelihood
        ...
        logLikelihood = loglikelihood_nodelay(parameters_c,incuData,mainControl)
        ...
        for tt in range(mcSteps):
            ...
            for ii in range(numPars):
                ...
                logLikelihood_new = loglikelihood_nodelay(parameters_mc,incuData,mainControl)
        ...
        #         alpha
                try:
                    ans = math.exp(logLikelihood_new+logIncuPrior_new-logLikelihood-logIncuPrior)
                except OverflowError:
                    ans = float('inf')
                alpha = min(1,ans)
                uu = rd()
...
for iiScn in range(len(recComplete)):
    if not os.path.isfile("figure3ef_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1)):
    #         create constant parameters
            mainControl = createConstants_nodelay(numCluster,numMeanIncubation,distrIncubation,clusterExposure)
    ...
    #     generation time
            ...
            rawPriorIncuData = random.lognormal(incuParaA,incuParaB,int(simParms[3]))
            rawIncu = random.lognormal(incuParaA,incuParaB,int(simParms[4]))
            rawGen = random.lognormal(genParaA,genParaB,int(simParms[4]))
            rawIncuData = rawIncu.copy()
            for i in range (math.ceil(simParms[4]*pre)):
                rawIncuData[i] = rawIncu[i]+rawGen[i]
            for i in range (math.ceil(simParms[3]*dis)):
                rawPriorIncuData[i] = rawPriorIncuData[i]+rawGen[i]
            # percentage of case
            caseUntil = math.ceil(len(rawIncuData)*simParms[2])
            orderIncuDelay = rawIncuData.copy()
            orderIncuDelay.sort()
            ...
            incuData = rawIncuData[rawIncuData<=dateUntil]
            if mainControl.distrIncubation == "Lognormal":
            ...
            meanIncu = 5
            stdIncu = 6
            x0 = [addSize,meanIncu,stdIncu]
            x0LowerBound = [0.01,0.01,0.01]
            x0UpperBound = [200,50,50]
            # test
            print(-loglikelihood_nodelay(x0,incuData,mainControl))
            xfmin = x0
            mcSteps = 2000
            stepSize = 0.2*np.array(xfmin)
            stepSize[stepSize<0.2] = 0.2
            out = mcmcParallel_nodelay(mcSteps,xfmin,incuData,mainControl,stepSize,x0LowerBound,x0UpperBound)
            # record
            burnIn = 0.5
            mcRes = [column[int(len(out[0])*burnIn):] for column in out][0]
            allRecTable = pd.DataFrame()
            # scnRec
            allRecTable[['mean_incubation','std_incubation','percent','prior_size','true_size']] = [simParms]
            # iiRec
            allRecTable[['date_until','max_time_span','num_infer_incubation']] = [dateUntil,maxTime,len(incuData)]
            allRecTable[['mode_add_size']] = statistics.mode([round(column[0]) for column in out[0]])
            # mcRes
            allRecTable[['add_size_50','add_size_2_5','add_size_97_5','mean_incubation_50','mean_incubation_2_5','mean_incubation_97_5','std_incubation_50','std_incubation_2_5','std_incubation_97_5']] = np.reshape(np.quantile(mcRes,[0.5, 0.025, 0.975],axis=0),(1,-1),order="F")
            allRecTable.to_csv("figure3ef_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1), index=False)
