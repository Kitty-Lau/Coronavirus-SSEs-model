...
rec = pd.read_csv("figure2ab_latin_hypercube_samples.csv",header=None)
...
for iiScn in range(len(recComplete)):
    if os.path.isfile("figure2ab_rec_summary/mcmc_res_all_rec_{}.csv".format(iiScn+1)):
        tempRec = pd.read_csv("figure2ab_rec_summary/mcmc_res_all_rec_{}.csv".format(iiScn+1))
        resultRec = resultRec.append(tempRec,ignore_index=True)
# resultRec
resultRec.to_csv("figure2ab_rec_summary.csv",index=False)
...
# plots
resultRec = pd.read_csv("figure2ab_rec_summary.csv")
