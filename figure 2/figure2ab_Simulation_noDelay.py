...
if not os.path.isfile("figure2ab_rec_latin_hypercube_samples.csv"):
    ...
    rec.to_csv("figure2ab_rec_latin_hypercube_samples.csv", header=None, index=False)
else:
    rec = pd.read_csv("figure2ab_rec_latin_hypercube_samples.csv",header=None)
...
for iiScn in range(len(recComplete)):
    if not os.path.isfile("figure2ab_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1)):
...
#     incubation
        if mainControl.distrIncubation == "Lognormal":
            ...
            rawIncuData = random.lognormal(incuParaA,incuParaB,int(simParms[4]))
        else:
            print ("Incubation period distribution misspecified.")
        # percentage of case
        ...
        allRecTable.to_csv("figure2ab_mcmc_res/mcmc_res_all_rec_{}.csv".format(iiScn+1), index=False)
